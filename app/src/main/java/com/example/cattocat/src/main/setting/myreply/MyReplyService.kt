...
class MyReplyService(val view: MyReplyView, val reply_id:Int) {
    fun tryGetPostSingle(){
        val myReplyRetrofitInterface = MyApplication.mRetrofit.create(MyReplyRetrofitInterface::class.java)

        myReplyRetrofitInterface.getMyReply(reply_id).enqueue(object:
            Callback<MyReplyResponse> {
            override fun onResponse(call: Call<MyReplyResponse>, response: Response<MyReplyResponse>) {
                view.onGetMyReplySuccess(response.body() as MyReplyResponse)
            ...
            override fun onFailure(call: Call<MyReplyResponse>, t: Throwable) {
